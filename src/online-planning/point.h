...
struct Point3D
{
	...
	double distXYSq(const Point3D& pt) const { return pow(x - pt.x, 2) + pow(y - pt.y, 2); }
	double distZAbs(const Point3D& pt) const { return abs(z - pt.z); }
	double distZ(const Point3D& pt) const { return z - pt.z; }
