...
namespace CommonUtils
{
	...
	inline void eraseIf(ContainerT& items, const PredicateT& predicate) {
	...
	/**
	 * Check whether given element in the given vector.
	 * 
	 * \param vec
	 * \param elem
	 * \return 
	...
	static bool isElemInVec(const vector<T, A>& vec, const T& elem) {
	 ...
	 */
	template <typename T, typename A>
	static bool isVecElemsUnique(const vector<T, A>& vec)
	{
	 ...
	 * \return
	 */
	static bool isBoolVecAllTrue(const vector<bool>& vec)
	{
	 ...
	 */
	template <typename T, typename A>
	static int findIndexByElem(const vector<T, A>& vec, const T member)
	{
	...
	template <typename T, typename A>
	static T vecMinElem(const vector<T, A>& vec)
	{
	...
	template <typename T, typename A>
	static T vecMaxElem(const vector<T, A>& vec)
	{
	...
	template <typename T, typename A>
	static T vecElemsSum(const vector<T, A>& vec)
	{
	...
	template <typename T, typename A>
	static T vecElemsMean(const vector<T, A>& vec)
	{
		if (vec.empty()) { return 0; }
		else if (vec.size() == 1) { return vec.back(); }
		T count = static_cast<T>(vec.size());
		return vecElemsSum(vec) / count;
	}
	 ...
	 */
	template <typename T, typename A>
	inline void vecEraseElem(vector<T, A>& vec, const T member)
	{
		vec.erase(std::remove(vec.begin(), vec.end(), member), vec.end());
	}

	template <typename T, typename A>
	inline void vecInsertElem(vector<T, A>& vec, size_t idx_in_vec, const T item2insert)
	{
...
namespace RandomUtils
{
	...
	template <typename T, typename A>
	static void vecUniformRandSampleOneElem
	(const std::vector<T, A>& vec_in, T& elem)
	{
		static std::random_device rd;
		static std::mt19937 engine(rd());
	...
	template <typename T, typename A>
	static void vecUniformRandSample(const vector<T, A>& vec_in,
	 ...
	 */
	template <typename T, typename A>
	static void vecUniformRandSampleExcludeFirstLast(const vector<T, A>& vec_in,
		const size_t num_elems, vector<T, A>& vec_out)
	{
		static std::random_device rd;
		static std::mt19937 engine(rd());
	 ...
	 */
	template<typename T, typename A>
	static void vecShuffleSkipFirstN(const size_t num_skip, vector<T, A>& vec)
	{
		static std::random_device rd;
		static std::mt19937 engine(rd());
	...
	template <typename T, typename A>
	static void vecBiasedSampleOneIndex(const vector<T, A>& vec_in,
	...
	template <typename T, typename A>
	static void vecBiasedSampleOneElem(const vector<T, A>& vec_in,
